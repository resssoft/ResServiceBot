...
func Provide(dispatcher *mediator.Dispatcher, logPath string) Application {
	client := &Client{
	...
	if err := dispatcher.Register(
		...
		mediator.FileLoggerEvents...); err != nil {
...
func (c *Client) Log(src string, data string, withoutTime bool, toDebug bool) {
	...
	if withoutTime {
		...
		row = fmt.Sprintf("[%s] %s \n", time.Now().Format(DateTimeFormat), data)
	}
	file.channel <- row
	if toDebug {
		log.Debug().Msg(data)
...
func (c *Client) AddSource(filePath, name string) error {
	file, err := os.OpenFile(c.LogPath+filePath, os.O_CREATE|os.O_APPEND|os.O_WRONLY, 0666)
	if err != nil {
		return err
	}
	c.mapSafety.Lock()
